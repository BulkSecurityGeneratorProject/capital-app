...
import au.com.normist.capital.repository.cap.dbutilsjpa.testutils.NonUpdatableEntity;
...
//    runner.save(entity);
//
//    verify(queryRunner).update("UPDATE SimpleEntity SET SimpleEntity.name = ? WHERE SimpleEntity.id = ?", name.get(), entity.getId());
//  }
  ...
  public void should_update_from_annotated_properties_if_entity_has_id() throws SQLException {
    ...
    verify(queryRunner).update("UPDATE myTableName SET myTableName.customDateColumn = ? WHERE myTableName.customNameId = ?", entity.getDate(), entity.getId());
  }
  ...
  public void should_ignore_non_updatable_column() throws SQLException {
    ...
    verify(queryRunner).update("UPDATE NonUpdatableEntity SET NonUpdatableEntity.name = ?, NonUpdatableEntity.notInserted = ? WHERE NonUpdatableEntity.id = ?", entity.getName(), entity.getNotInserted(), entity.getId());
  }
