...
import au.com.normist.capital.repository.cap.dbutilsjpa.testutils.EntityWithJoin;
...
public class JpaBeanProcessor_JoinsTest {
  ...
  public void should_join_many_to_one() throws Exception {
    Integer simpleEntityId = queryRunner.insert(conn, insert(SimpleEntity.class).columns("name").sql(), new ScalarHandler<Integer>(), "my name");
    ...
    String sql = "SELECT EntityWithJoin.*, SimpleEntity.* FROM EntityWithJoin, SimpleEntity WHERE EntityWithJoin.simple_id = SimpleEntity.id AND EntityWithJoin.id = ?";
    EntityWithJoin entityWithJoin = queryRunner.query(conn, select().from(EntityWithJoin.class).where().eq("id").from(SimpleEntity.class).sql(), beanHandler, entityWithJoinId);
